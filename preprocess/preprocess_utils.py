...
import tqdm
import pdb
from preprocess.ICU_class import ICU
import re
...
def name_dict(df, table_name, config, src, column_select):
    ...
    if table_name in file_dict:
        ...
        path = '/media/qlunlp/86f9f306-e54a-46ad-b338-2b9f8634686f/qluai/Zhang/Chet_integrate/data/%s/' % (src) #/home/ubuntu/zx/Chet_events/data/mimic4
        dict_path = os.path.join(path, dict_name+'.csv')
        code_dict = pd.read_csv(dict_path)
        key = code_dict['ITEMID'] #mimic4: itemid ITEMID
        value = code_dict['LABEL'] #mimic4: label LABEL
...
def get_time_bucket(icu, args, src):
    ...
    for event in icu.events:
        if src=='eicu':
            ...
            #print('event.event_time',event.event_time)
            pattern = '%y-%m-%d %H:%M:%S'
            try:
               #date_time = strftime(pattern,event.event_time)
               event_time = pd.to_datetime(event.event_time).to_datetime64()
            except:
                print('event.event_time:{}  type:{} \n'.format(event.event_time,type(event.event_time)))
            event_time = pd.to_datetime(event_time).to_datetime64()
