...
def Table2event_multi(row, columns, table_type, column_select):
    ...
    #time = row['TIME']
    value_Table2event_multi = round(row['value'], 4) if column_select == True and type(row['value']) in ['float', 'int'] else 0
...
def set_admission():
    ...
    cols = {pid_col: 'SUBJECT_ID', adm_id_col: 'HADM_ID'} #SUBJECT_ID HADM_ID在mimic4是subject_id; HADM_ID在mimic4是hadm_id
    converter = {
        'SUBJECT_ID': int,
        'HADM_ID': int,
        'DMITTIME': lambda cell: datetime.strptime(str(cell), '%Y-%m-%d %H:%M:%S') #ADMITTIME 在mimic4中是admittime
    }
...
def icu_class_gen(src, config):
    ...
    #MIMIC-IV admissions
    icu = pd.read_csv('/media/qlunlp/86f9f306-e54a-46ad-b338-2b9f8634686f/qluai/Zhang/Chet_integrate/mimic3/raw/ADMISSIONS.csv', usecols=list(cols.values()), converters=converters)
    print("event_icu filepath:",'/media/qlunlp/86f9f306-e54a-46ad-b338-2b9f8634686f/qluai/Zhang/Chet_integrate/mimic3/raw/ADMISSIONS.csv')
    ...
    for idx in range(len(config['Table'][src])):
        ...
        df_path = os.path.join('/media/qlunlp/86f9f306-e54a-46ad-b338-2b9f8634686f/qluai/Zhang/Chet_integrate/data/', 'mimic3', table_name+'.csv')
