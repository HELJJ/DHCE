...
tokenizer = AutoTokenizer.from_pretrained("/media/qlunlp/86f9f306-e54a-46ad-b338-2b9f8634686f/qluai/Zhang/Chet/Clinical_diagnosis")
...
class ColContent:
    ...
    #value_tok_id : List[int] = field(default_factory=List)
    content_col_content_id: List[int] = field(default_factory=list)
    ...
    def __post_init__(self):
        self.content_tok_id = tokenizer.encode(self.content)[1:-1] if type(self.content) is str else self.content #搞清楚self.content是什么，self.col是什么 然后合并组成形式
        content_tok_id = tokenizer.encode(self.content)[1:-1] if type(self.content) is str else self.content #搞清楚self.content是什么，self.col是什么 然后合并组成形式

        self.col_tok_id = tokenizer.encode(self.col)[1:-1] if type(self.col) is str else self.col
        #col_tok_id = tokenizer.encode(self.col)[1:-1] if type(self.col) is str else self.col
        # self.value_tok_id = tokenizer.encode(self.value)[1:-1] if type(self.value) is float else self.value
        # self.content_col_value_id = [101]+self.col_tok_id+[102]+self.content_tok_id+[102]+self.value_tok_id+[102]
        ...
        self.table_tok_id = tokenizer.encode(self.table)[1:-1]
        #self.content_col_content_id = col_tok_id + content_tok_id
        self.content_col_content_id =  content_tok_id
...
class Event:
    ...
    content_value : List[int] = field(default_factory=list)
