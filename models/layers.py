...
class GraphLayer(nn.Module):
    ...
    def forward(self, code_x, neighbor, c_embeddings, n_embeddings):
        center_codes = torch.unsqueeze(code_x, dim=-1)#center_codes第t次就诊的diagnose 对应论文的mt
        neighbor_codes = torch.unsqueeze(neighbor, dim=-1)
        center_codes = center_codes.float()
        center_embeddings = center_codes * c_embeddings #对应论文 mt对位相乘M
        neighbor_codes = neighbor_codes.float()
        ...
        co_embeddings = self.activation(self.dense(center_embeddings + cc_embeddings + cn_embeddings)) #聚合有关诊断节点的上下文
        no_embeddings = self.activation(self.dense(neighbor_embeddings + nn_embeddings + nc_embeddings)) #聚合有关邻居节点的上下文
...
class TransitionLayer(nn.Module):
    ...
    def forward(self, t, co_embeddings, divided, no_embeddings, unrelated_embeddings, hidden_state=None):
        #m1为persistent diseases;m2为Emerging neighbors;m3为Emerging unrelated diseases
