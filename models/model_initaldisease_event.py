...
class Model(nn.Module):
    ...
    def forward(self, code_x, divided, neighbors, lens, events, input_ids, code_mask, diagnosis_list):
        ...
        '''
        for code_x_i, divided_i, neighbor_i, len_i, events_i, diagnosis_list_i \
                in zip(code_x, divided, neighbors, lens, events, diagnosis_list):

            no_embeddings_i_prev = None
            output_i = []
            h_t = None
            for t, (c_it, d_it, n_it, len_it, events_it, diagnosis_list_it) in enumerate(zip(code_x_i, divided_i, neighbor_i, range(len_i), events_i, diagnosis_list_i)):
                ...
                #combined_vector = torch.cat([event_output, output_it], dim=0)
                    ...
                    event_output_sum = torch.sum(event_output, dim=0, keepdim=True)
                    '''
                    event_output_sum = torch.sum(event_output, dim=0, keepdim=True)  # 沿着第一个维度相加，将(n,512)变成(1,512)
                    output_it = output_it.unsqueeze(0)
                    #combined_vector = torch.cat((output_it, events_it), dim=0)
                    F = self.sigmoid(
                        self.dense_event(event_output_sum) + self.dense_visitpre(output_it) + self.b_gate)  # 门控机制
                    # 学习visit representation和event representation重要程度
                    combined_vector = F * output_it + (1 - F) * event_output_sum
                    combined_vector = combined_vector.squeeze(0)
                    '''
                    output_it = torch.cat((output_it.unsqueeze(0), event_output_sum), dim=0)
